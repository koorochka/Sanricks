...
function updateBasketTable(basketItemId, res)
{
			...
			for (i = 0; i < arColumns.length; i++)
			{
				switch (arColumns[i])
				{
					...
					case 'PRICE':
						...
						oCellPrice.innerHTML = '<div class="current_price" id="current_price_' + arItem['ID'] + '">' + arItem['PRICE'] + '</div>' +
	...
	if (!!res.BASKET_DATA)
	{
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('current_summ_' + id))
					BX('current_summ_' + id).innerHTML = (item.PRICE*item.QUANTITY).toFixed(2);

				if (BX('old_price_' + id))
					BX('old_price_' + id).innerHTML = (item.DISCOUNT_PRICE_PERCENT > 0) ? item.FULL_PRICE : '';
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED'))
			BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum'];
		...
		var count_counts = 0;
		$('.input--counter').each(function(key, value){
			count_counts += Number($(value).val());
		});
		$('#count__num').html(count_counts);
		...
		onsuccess: function(result)
		{
			...
			if(params.coupon)
			{
				...
				if(!!result && !!result.BASKET_DATA && !!result.BASKET_DATA.NEED_TO_RELOAD_FOR_GETTING_GIFTS)
				{
                    ...
                    $("#gifts-cupon").find(".color-orange").remove();
                    $("#gifts-cupon").prepend($("<h4 />", {
                        class: "color-orange text-center",
                        html: "Скидочный купон не найден"
                    }));
				}
